...
def my_threshold(img, win=5, beta=0.9):
    ...
    means = sums // cnts
    img = np.where(img < means * beta, img - 255, 255)
...
def for_test():
    org_img = imread("process_img/process_img.png")
    u2net_img = imread("process_img/result/process_img.png")
    ...
    show_img(morphologyEx_open(transform_img1))
...
def main(org_img):
    """
    org_img: 已经读取的图片 BGR -> RGB  最好灰度图， 假设是灰度图
    """
    print(org_img)
    # org_img = imread("./process_img.png")
    # show_img(org_img)
    ##
    # u2net_img = imread("./3.png")
    ##
    # 灰度图
    # (有个想法，自定义将彩色图片转为灰度图，因为图片是灰白居多，应对特定场景)
    org_img = gray(org_img)
    #
    cv2.imwrite("./process_img/process_img.png", org_img)
    try :
        u2net_main.main()
    except :
        pass

    process_img = cv2.imread(r".\process_img\result\process_img.png", cv2.IMREAD_GRAYSCALE)
    # .......
    # 形态学处理、或u2net处理
    ...
    ret, Threshold_img = cv2.threshold(process_img, 150, 255, cv2.THRESH_TRUNC)
    ...
    transform_img1 = perspective_transform(org_img, contours)  # upload_img
    show_img(transform_img1)
    fiall_img1 = my_threshold(morphologyEx_open(transform_img1))
    fiall_img2 = morphologyEx_open(my_threshold(transform_img1))
    # 上传
    uid, respone = upload.main()
    # 删除
    os.remove("process_img/process_img.png")
    os.remove("process_img/result/process_img.png")
    show_img(fiall_img1)
    show_img(fiall_img2)
    return uid, respone
...
if __name__ == "__main__":
    uid, respone = main(cv2.imread("3_.png"))
    print(uid)
